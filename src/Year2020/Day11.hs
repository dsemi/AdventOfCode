...
adjacentsOccupied :: [Coord] -> Grid -> Int
adjacentsOccupied adjs grid = length $ filter (=='#') $ map (grid !) adjs
...
next grid xy = (xy, v)
    where v | grid ! xy == 'L' && numOcc == 0 = '#'
            | grid ! xy == '#' && numOcc >= 4 = 'L'
            | otherwise = grid ! xy
          numOcc = adjacentsOccupied adjs grid
          adjs = filter (inRange (bounds grid)) $ map (+xy) dirs
          dirs = [ V2 (-1) (-1), V2 0 (-1), V2 1 (-1)
                 , V2 (-1) 0, V2 1 0
                 , V2 (-1) 1, V2 0 1, V2 1 1 ]

stabilize :: (Grid -> Coord -> (Coord, Char)) -> Grid -> Int
stabilize f grid = length $ filter (=='#') $ elems $ head $ catMaybes
                   $ zipWith (\a b -> if a == b then Just a else Nothing) grids $ tail grids
